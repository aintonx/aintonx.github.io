...
> Обновление Phase 1C: read-only каталог уже подключён во frontend через
> `catalog/list`. Этот Phase 1B-документ оставлен как исторический handoff.
> Для актуальной работы смотри `docs/db-phase1c-readonly-catalog.md`.
> Заказы, резервы, YooKassa, Object Storage и business-write маршруты всё ещё
> выключены.
