...
/* ── Запуск ──────────────────────────────────────────────────────────────── */
if (режим === 'diff') {
  /* СНАЧАЛА снимаем, потом сравниваем. Раньше --diff только сличал то, что
     уже лежало в tools/shots/current, и если забыть отдельно запустить
     `npm run shots`, сравнение шло со СТАРЫМИ кадрами и уверенно печатало
     «значимых изменений нет» на любую правку. Это давало ложную гарантию —
     худший вид проверки. Теперь пересъёмка встроена и пропустить её нельзя. */
  await снять();
